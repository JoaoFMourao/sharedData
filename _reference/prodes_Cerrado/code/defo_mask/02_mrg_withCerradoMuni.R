...
aux.name <- load(
  file.path(
    ...
    "cleanData", 
    "defo_mask",
    "cln_lcv_dfrst_mask_cerrado_biome_prodes_inpe_sf.Rdata"
  )
)
...
muni_biome = muni_biome %>% filter(biomeCerrado == TRUE) #Only the cerrado munis 
  ...
  if(length(result$prodes_id) != 0) {
    ...
    save(result,
         file = file.path(
           ...
           "defo_mask", 
           "02_mrg_cerradoMuni",
           paste0("prodes_mask_cerrado_muni_",
                  municipality,
                  "_sf.Rdata")
         )
    )
...
# Time difference around 35 min - 151 
