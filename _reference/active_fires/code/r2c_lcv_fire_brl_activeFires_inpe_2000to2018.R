...
#
# > THIS SCRIPT
# AIM: TREATS RAW INPE DATA ON BURNINGS -- 2000 to 2018
...
# AUXILIARY LIST
# creates a list with all folders names of input directory from 2000 to 2018
aux.folder.names <- list.files(file.path(DIR.CDR.DATA, "raw2clean/landCover/fire/brazil/activeFires_inpe/input"))
aux.folder.names <- aux.folder.names[1:19]
...
for (i in seq_along(aux.folder.names)) {
  ...
  for (j in seq_along(aux.files.names)) {
    ...
    # exploration
    MissingAllColumnsAndRowsSp(x = fires) # variables 'DiaSemCh', 'Precipit' and 'RiscoFog' are full missing from 2000 to 2016; 
                                          # variable 'FRP' is full missing from 2000 to 2017;
                                          # variable 'AreaIndu' is full missing in the whole database
...
# STOP THE CLOCK
proc.time() - ptm   # around 1hs and 30min of process (~5600sec)
...
new.names <- c("date", "satellite", "country", "state", "municipality", "biome", "days_wt_rain",
               "precipitation", "fire_risk", "lat", "lon", "area_indu", "frp")
...
# PREP
return.obj <- c("cln.lcv.fire.brl.activeFires.inpe.2000to2018")
