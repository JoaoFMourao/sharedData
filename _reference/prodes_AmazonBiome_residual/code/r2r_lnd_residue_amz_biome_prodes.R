...
#
# > THIS SCRIPT
# AIM: TREAT RAW DATA [DEFORESTATION RESIDUALS POLYGONS ONLY]
...
raw.prodes <- st_read(file.path(DIR.CPI.DATA, 
                                'land/prodes_AmazonBiome_residual/rawData',
                                'residual_biome.shp'))
...
# translation
# prodes_class
#raw.prodes%>%st_drop_geometry() %>% distinct(prodes_class)
raw.prodes$prodes_class <- gsub(pattern = "^RESIDUO$", replacement = "residue", x = raw.prodes$prodes_class)   
# raw.prodes%>%st_drop_geometry() %>% distinct(prodes_class)
...
cln.lcv.rsd.amz.biome.prodes.inpe.sf <- raw.prodes
rm(raw.prodes)
...
#summary(cln.lcv.rsd.amz.biome.prodes.inpe.sf)
#View(cln.lcv.rsd.amz.biome.prodes.inpe.sf)
#plot(cln.lcv.rsd.amz.biome.prodes.inpe.sf)
...
save(cln.lcv.rsd.amz.biome.prodes.inpe.sf, 
     file = file.path(DIR.CPI.DATA, 
            'land/prodes_AmazonBiome_residual/cleanData',
