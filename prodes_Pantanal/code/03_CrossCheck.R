...
defo.biome <- file.path(
  DIR.CPI.DATA,
  "land/prodes_Pantanal/cleanData/02_mrg_pantanalMuni"
)
...
calculate.defo <- function(municipality) {
  ...
  load(
    file.path(
      defo.biome,
      paste0("prodes_inc_pantanal_muni_",
             municipality,
             "_sf.Rdata")
    )
  )
...
aux.name <- load(
  file.path(
    DIR.CPI.DATA,
    "land",
    "prodes_Pantanal\\cleanData",
    "cln_lcv_dfrst_pantanal_biome_prodes_inpe_sf.Rdata"
  )
)
...
write.xlsx(answer,file.path(
  DIR.CPI.DATA,
  "land/prodes_Pantanal/cleanData","03_crossCheck.xlsx"))
