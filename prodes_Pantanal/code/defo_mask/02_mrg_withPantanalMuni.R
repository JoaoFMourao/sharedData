...
aux.name <- load(
  file.path(
    ...
    "cleanData", 
    "defo_mask",
    "cln_lcv_dfrst_mask_pantanal_biome_prodes_inpe_sf.Rdata"
  )
)
...
muni_biome = muni_biome %>% filter(biomePantanal == TRUE) #Only the Pantanal munis 
  ...
  if(length(result$prodes_id) != 0) {
    ...
    save(result,
         file = file.path(
           ...
           "defo_mask",
           "02_mrg_pantanalMuni",
           paste0("prodes_mask_pantanal_muni_",
                  municipality,
                  "_sf.Rdata")
         )
    )
...
# Time difference of 12.75285 secs