...
# read
 aux.sp <- st_read(dsn   = file.path(DIR.CPI.DATA, "propertyRights/public_Forests/rawData/2022/"), 
                   layer = paste0("CNFP_2022_",aux.states[i])) 
...
cln.prp.lndTe.brl.publicForests.sfb.2022 <- publicForest.BR
rm(publicForest.BR)


cln.prp.lndTe.brl.publicForests.sfb.2022 <- cln.prp.lndTe.brl.publicForests.sfb.2022 %>% 
  rename(uf = state_uf)
...
save(cln.prp.lndTe.brl.publicForests.sfb.2022,
     file = file.path("A:/propertyRights/public_Forests/cleanData", 
                      "cln_prp_lndTe_brl_publicForests_sfb_2022_sf.Rdata"))
