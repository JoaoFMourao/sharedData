...
aux.name <- load(
  file.path(
    ...
    "cleanData", 
    "defo_mask", 
    "cln_lcv_dfrst_mask_pampa_biome_prodes_inpe_sf.Rdata"
  )
)
...
muni_biome = muni_biome %>% filter(biomePampa == TRUE) #Only the pampa munis 
  ...
  if(length(result$prodes_id) != 0) {
    ...
    save(result,
         file = file.path(
           ...
           "defo_mask", 
           "02_mrg_pampaMuni",
           paste0("prodes_mask_pampa_muni_",
                  municipality,
                  "_sf.Rdata")
         )
    )
...
# Time difference of 5.480036 mins