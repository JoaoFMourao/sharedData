...
ExcludeMissing_sf <- function(x, margin = 1) {
  ...
  if (inherits(x, "SpatialPointsDataFrame") |inherits(x, "SpatialPolygonsDataFrame")) {
    stop("Can't BE sp SpatialPointsDataFrame OR SpatialPolygonsDataFrame CLASS OBJECT")  # returns error if object not spatial
  }
  
  na.index <- unique(as.data.frame(which(is.na(x), arr.ind = TRUE))[, margin])  # records margin index of NA occurrence
