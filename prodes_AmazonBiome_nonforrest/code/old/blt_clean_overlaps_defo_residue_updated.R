...
muni = unique(str_extract(files.nonforest,pattern = "\\d{7}")) 

# START FUNCTION  ----------------------------------------------------------------------------------------------------------------------------------------

fixing_overlap = function(municipality){
   print(paste0("cleaning muni ", municipality))

    # Filter the big PRODES muni path that match the municipality 
    file_muni <- files.nonforest[grep(municipality, files.nonforest)]
    
    # load data 

    muni = get(load(file_muni)) 
    
    
    deforestation <- muni %>% filter(prodes_main_class == "deforestation")
    residue <- muni %>% filter(prodes_main_class == "residue")
    ...
    for (i in seq_len(nrow(deforestation))) {  
      ...
      save(muni,
           file = file.path(paste(
             ...
             'blt_nonforest_without_overlaps_', 
             municipality, '.Rdata', sep=''))
      )
} 

# RUN EVERYTHING #####
plan(multisession, workers = 8)#detectCores()-2)
res = future_lapply(muni, fixing_overlap)
